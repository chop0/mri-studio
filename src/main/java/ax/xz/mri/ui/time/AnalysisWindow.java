...
/**
 * The measurement-averaging range used by analysis panes. Independent of both
 * the cursor and the viewport — the cursor can sit outside it; the viewport
 * never drags it. Self-clamps to {@code [0, domain.maxTime()]} with a minimum
 * span of {@link #MIN_SPAN}.
 */
public final class AnalysisWindow {
    ...
    public AnalysisWindow(Domain domain) {
        this.domain = domain;
    ...
    public void moveStart(double t) {
        start.set(t);
    }

    public void moveEnd(double t) {
        end.set(t);
    }

    public void moveBy(double deltaTime) {
