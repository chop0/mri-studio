...
public final class EditSession {
    ...
    // ── Selection ────────────────────────────────────────────────────────────
    /** Selection state — the new UI binds CSS pseudo-classes against this. */
    public final SelectionModel selection = new SelectionModel();
    /** Selected clip ids; alias for {@link Selectable#selected()}. */
    public final ObservableSet<String> selectedClipIds = selection.selected();
    /** Most recently clicked clip — drives the inspector. */
    public final ObjectProperty<String> primarySelectedClipId = selection.primary();
    /** In-progress gesture observable; the new timeline element library binds against it. */
    public final EditPreview preview = new EditPreview();
    ...
    // ── Time axis (μs) ──────────────────────────────────────────────────────
    /**
     * Shared time-axis state — cursor, viewport, analysis window, domain.
     * Production wires the studio's global {@link TimeAxis} via
     * {@link #setTimeAxis}; tests get a private default instance so they
    ...
    /** The simulation-config document the editor is bound to. */
    ...
    // ── Snapping ─────────────────────────────────────────────────────────────
    /**
     * Layout-snap settings. The {@link SnapModel} computes the snap candidates
     * lazily — it walks the live {@link #clips} list each call rather than
     * caching, so an in-progress drag finds itself in the candidate list and
     * the snap stays consistent.
     */
    public final SnapModel snap = new SnapModel(
        () -> clips.stream().<Double>mapMulti((c, sink) -> {
            sink.accept(c.startTime());
            sink.accept(c.endTime());
        }).toList(),
        () -> timeAxis == null ? 1000 : timeAxis.viewport.span()
    );
    /** Alias for {@link SnapModel#enabled}. */
    public final BooleanProperty snapEnabled = snap.enabled;
    /** Alias for {@link SnapModel#gridSize}. */
    public final DoubleProperty snapGridSize = snap.gridSize;
    ...
    public TimeAxis timeAxis() { return timeAxis; }

    /** Wire the studio's shared time axis. Production calls this once after construction. */
    public void setTimeAxis(TimeAxis timeAxis) {
        this.timeAxis = timeAxis;
        timeAxis.domain.maxTime.set(totalDuration.get());
    }
    ...
    public EditSession() {
        ...
        // Mirror the hardware-config behaviour for sim probes: auto-enable
        // every probe defined by the active circuit when a config is bound,
        // so the OutputBand shows traces by default without forcing the user
        // to discover the Outputs popover.
        activeConfig.addListener((obs, o, n) -> {
            var circuit = activeCircuit();
            if (circuit == null) return;
            var validProbes = new LinkedHashSet<String>();
            for (var probe : circuit.probes()) validProbes.add(probe.name());
            // OpticalCounter is a first-class output channel — without this the
            // NV-diamond circuit's "Red counter" never lights up in the output
            // band, even though the simulator produces a clicks_red trace.
            for (var counter : circuit.opticalCounters()) validProbes.add(counter.name());
            enabledSimOutputs.removeIf(name -> !validProbes.contains(name));
            enabledSimOutputs.addAll(validProbes);
        });
    }
    ...
    public void open(SequenceDocument document) {
        ...
        try {
            ...
            timeAxis.domain.maxTime.set(totalDuration.get());
            timeAxis.viewport.setSpan(0, viewportFitTo(totalDuration.get()));
    ...
    private void rehydrate(SequenceDocument doc) {
        if (doc == null) return;
        // Self-dispatched changes don't need an echo: we just produced this
        // doc, our observables are already in the matching state. Skipping
        // saves a full currentDoc() snapshot per mutate() call (two
        // List.copyOf + a fresh ClipSequence + SequenceDocument).
    ...
    /**
     * Diagnostic counter — incremented on every {@link #currentDoc} invocation.
     * Used by perf-regression tests to assert that {@link #mutate} doesn't
     * compute the {@code beforeDoc} snapshot inside an active transaction
     * (which is dead work — the transaction commits only the final state).
     */
    private int currentDocCallCount;

    /** @return the {@link #currentDoc} call count since session construction. */
    public int currentDocCallCount() { return currentDocCallCount; }
    ...
    /** Duplicate a track (with a new id and "(copy)" suffix); does not duplicate clips. */
    public void duplicateTrack(String trackId) {
        var src = findTrack(trackId);
        if (src == null) return;
        int idx = indexOfTrack(trackId);
        var dup = src.withNewId().withName(src.name() + " (copy)");
        mutate(() -> tracks.add(idx + 1, dup));
    }
    ...
    public boolean isSelected(String clipId) { return selection.isSelected(clipId); }

    public List<SignalClip> selectedClips() {
        return clips.stream().filter(c -> selection.isSelected(c.id())).toList();
    }

    public SignalClip primarySelectedClip() { return findClip(selection.primary().get()); }

    public void selectOnly(String clipId) { selection.selectOnly(clipId); }

    public void addToSelection(String clipId) { selection.add(clipId); }

    public void toggleSelection(String clipId) { selection.toggle(clipId); }
    ...
    public void resizeClipLeft(String clipId, double newStartTime) {
        ...
        mutate(() -> clips.set(idx, new SignalClip(
            ...
            finalMediaOffset, finalMediaDuration, clip.stayCentred()
        )));
    }

    public void resizeClipRight(String clipId, double newEndTime) {
        int idx = indexOfClip(clipId);
        if (idx < 0) return;
        var clip = clips.get(idx);
        double newDuration = Math.max(dt.get(), newEndTime - clip.startTime());
        mutate(() -> clips.set(idx, clip.withDuration(newDuration)));
    }
    ...
    // ── Clipboard ops + select-all ──────────────────────────────────────

    /** Shared system-clipboard channel for clips. One instance per JVM —
     *  paste targets and copy callers all bind to this same {@link DataFormat}. */
    public static final ClipboardChannel<SignalClip> CLIP_CLIPBOARD =
        new JsonClipboardChannel<>("clips", SignalClip.class);

    /** Put the current selection on the system clipboard. */
    public boolean copySelectedClips() {
        var sel = selectedClips();
        return !sel.isEmpty() && CLIP_CLIPBOARD.put(java.util.List.copyOf(sel));
    }

    /** Copy then delete the current selection. */
    public boolean cutSelectedClips() {
        if (selectedClipIds.isEmpty()) return false;
        boolean ok = copySelectedClips();
        if (ok) deleteSelectedClips();
        return ok;
    }

    /** Paste at the current cursor time, on each clip's original track. */
    public void pasteAtCursor() {
        pasteAtTime(timeAxis == null ? 0.0 : timeAxis.cursor.time.get(), null);
    }

    /**
     * Paste clips offset so the earliest start lands at {@code tMicros}.
     * If {@code targetTrackId} is non-null, every pasted clip is rerouted
     * to that track (handy from a TrackLane right-click "Paste here");
     * otherwise clips keep their original tracks.
     */
    public void pasteAtTime(double tMicros, String targetTrackId) {
        var pasted = CLIP_CLIPBOARD.peek();
        if (pasted.isEmpty()) return;
        double earliest = pasted.stream().mapToDouble(c -> c.startTime()).min().orElse(0);
        double delta = tMicros - earliest;
        mutate(() -> {
            var newIds = new java.util.LinkedHashSet<String>();
            for (var c : pasted) {
                var dupe = c.withNewId().withStartTime(Math.max(0, c.startTime() + delta));
                if (targetTrackId != null) dupe = dupe.withTrack(targetTrackId);
                clips.add(dupe);
                newIds.add(dupe.id());
            }
            selectedClipIds.clear();
    ...
    /** Select every clip in the active sequence. */
    public void selectAllClips() {
        if (clips.isEmpty()) return;
        var ids = clips.stream().map(c -> c.id()).toList();
        mutate(() -> {
            selectedClipIds.clear();
            selectedClipIds.addAll(ids);
            primarySelectedClipId.set(ids.get(0));
        });
    }
    ...
    public void setTotalDuration(double duration) {
        ...
        mutate(() -> {
            ...
            timeAxis.domain.maxTime.set(duration);
        });
    }
    ...
    /** Delegates to {@link SnapModel#snapTime(double)}. */
    public double snapTime(double rawTime) { return snap.snapTime(rawTime); }

    /**
     * Pick a sensible initial viewport end: prefer the bounding box of the
     * loaded clips (with a 20% margin), but never zoom out past the
     * sequence's total duration. CPMG-style sequences have a ~5 second
     * total but the actual RF/gradient activity covers the first ~50 ms;
     * fit-to-total leaves the user staring at clips that are sub-pixel-wide
     * pixel-strips.
     */
    private double viewportFitTo(double total) {
        double activeEnd = 0;
        for (var clip : clips) activeEnd = Math.max(activeEnd, clip.endTime());
        if (activeEnd <= 0) return Math.max(1, total);
        double padded = activeEnd * 1.2;
        return Math.min(Math.max(padded, total * 0.05), total);
    }
    ...
    public SignalClip createDefaultClip(ClipKind kind, String trackId, double startTime) {
        double clipDuration = Math.max(dt.get() * 5, timeAxis.viewport.span() * 0.15);
    ...
    /**
     * Roll back any active transaction — restores the document to its
     * pre-transaction state and produces no undo entry. No-op if none is open.
     * The post-condition is that the underlying state manager re-emits the
     * pre-transaction document so observers re-hydrate the rolled-back values.
     */
    ...
    /**
     * Run an undoable mutation. Bumps revision, then dispatches the change
     * through the active transaction (if any) or the editor's undo log.
     *
     * <p>Two snapshot optimisations land here, both critical for 60-fps drag:
     * <ul>
     *   <li><b>Skip {@code beforeDoc} inside a transaction.</b> The transaction
     *       commits only the FINAL state, so the per-frame
     *       {@code beforeDoc}/{@code Objects.equals} no-op skip is dead work.
     *       Saves two {@code List.copyOf} calls and an O(N) deep equals per
     *       drag step.
     *   <li><b>Suppress the rehydrate echo for self-dispatched changes.</b>
     *       After {@code activeTx.apply}/{@code editor.apply} the editor's
     *       valueProperty fires its listener which calls {@link #rehydrate}
     *       which calls {@link #currentDoc} again to compare live vs. echoed
     *       state. We just produced this state — no need to compare. The
     *       {@code selfDispatching} flag short-circuits the echo.
     * </ul>
     */
    private boolean selfDispatching;
    ...
    private void mutate(Runnable change) {
        ...
        var beforeDoc = activeTx != null ? null : currentDoc();
        change.run();
        bumpRevision();
        if (editor == null) return;
        ...
        try {
            ...
            } else if (!Objects.equals(beforeDoc, afterDoc)) {
