...
/**
 * r-z cross-section: colour-coded |M⊥| or signal at the cursor time.
 * Port of {@code drawCrossSection()} from draw/crossSection.ts.
 */
public class CrossSectionPane extends CanvasPane {
    ...
    public CrossSectionPane(AppState s) {
        super(s);
        installMouseHandlers();
        onAttached();
    }

    @Override public String getPaneId()    { return "cross-section"; }
    @Override public String getPaneTitle() { return "Cross Section"; }
    ...
    protected void installMouseHandlers() {
        ...
        canvas.setOnContextMenuRequested(e -> {
            ...
            var rz   = screenToRZ(e.getX(), e.getY());
            var add  = new MenuItem(String.format("Add here (r=%.1f, z=%.1f)", rz[0], rz[1]));
            ...
            var clear = new MenuItem("Clear all");
    ...
    protected void paint(GraphicsContext g, double w, double h) {
        ...
        double zMax = f.zMm[f.zMm.length - 1];
        ...
        var zSamples = buildZSamples(zMax);
        ...
        for (int ir = 0; ir < RADIAL_SAMPLES; ir++) {
            ...
            for (double z : zSamples) {
                ...
                double mp  = st.mPerp();
                double ph  = st.phaseDeg();
                sumMx += st.mx(); sumMy += st.my();
                cells.add(new CellSample(st.mx(), st.my(), mp, ph, r, z));
            }
        }
        ...
        g.setStroke(Color.color(0.13, 0.77, 0.37, 0.6)); g.setLineWidth(1);
        g.setLineDashes(4, 3);
        g.strokeLine(pad_l, yTop, pad_l + pW, yTop);
        g.strokeLine(pad_l, yBot, pad_l + pW, yBot);
        g.setLineDashes();
        g.setFill(Color.color(0.13, 0.77, 0.37, 0.6));
        g.setFont(MONO_BOLD_7);
        g.fillText("slice", pad_l + 2, (yTop + yBot) / 2 + 3);

        // Axes
        g.setStroke(Color.color(1, 1, 1, 0.12)); g.setLineWidth(0.5);
        g.beginPath(); g.moveTo(pad_l, pad_t); g.lineTo(pad_l, pad_t + pH);
        g.lineTo(pad_l + pW, pad_t + pH); g.stroke();

        // Z ticks
        g.setFill(TX2); g.setFont(MONO_7); g.setTextAlign(TextAlignment.RIGHT); g.setGlobalAlpha(0.7);
        ...
        for (double z = -Math.floor(xsH / zTickStep) * zTickStep; z <= xsH; z += zTickStep) {
            ...
            g.setStroke(Color.color(1, 1, 1, 0.05)); g.setLineWidth(0.3);
        ...
        // Axis labels
        g.setFill(TX2); g.setFont(MONO_BOLD_7); g.setGlobalAlpha(0.5);
        g.setTextAlign(TextAlignment.CENTER);
        ...
        for (var iso : appState.isochromats.isochromats) {
            ...
            g.setFill(iso.colour()); g.setGlobalAlpha(iso.visible() ? 0.9 : 0.1);
            g.fillOval(px - 4, py - 4, 8, 8);
            g.setStroke(Color.color(0, 0, 0, 0.6)); g.setLineWidth(1);
            g.setGlobalAlpha(iso.visible() ? 0.7 : 0.1);
