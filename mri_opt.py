...
def optimize_masked_controls(
    ...
    bar = tqdm(total=config.opt_steps, desc="L-BFGS-B", unit="iter", leave=False)
    ...
    def fg(x_free):
        ...
        return value, grad_full[free_idx]

    def callback(xk):
        state["nit"] += 1
        bar.update(1)
        x_full = merge_free(xk)
        if state["nit"] % max(config.snapshot_every, 1) == 0:
            snapshots[state["nit"]] = x_full.copy()
        if progress_fn is not None and config.print_every > 0 and state["nit"] % config.print_every == 0:
        ...
        if current_best is not None:
            bar.set_postfix(best=float(current_best))
